...
export default function PaymentScreen({ event, quantity, onBack, onPay }) {
  ...
  const handlePayNow = (e) => {
    e.preventDefault();
    // Loosened validation as requested so any fake information completes checkout
    setFormError('');
    onPay(total);
  };
  ...
  return (
    <div style={{
      ...
      >
        ...
        {/* Event Card Summary with Qty Badge */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          gap: '12px',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px', width: '100%', boxSizing: 'border-box' }}>
          ...
          {/* Card inputs (Disabled for free events) */}
          {event.price > 0 ? (
